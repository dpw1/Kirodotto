...
export class UIManager {
  ...
  createLifeIcons() {
    ...
    for (let i = 0; i < CONFIG.lives; i++) {
      ...
      const lifeIcon = this.scene.add.text(x, startY, '❤️', {
        fontSize: '24px'
      }).setOrigin(0.5);
  ...
  createPauseButton() {
    ...
    this.pauseButton = this.scene.add.text(x, y, '⏸️', {
      fontSize: '32px'
    }).setOrigin(0.5);
  ...
  createPausePanel() {
    ...
    this.pausePanel.add([panelBg, title, resumeButton, restartButton, menuButton]);
  ...
  showLevelComplete(level, score, callback) {
    ...
    const titleText = this.scene.add.text(0, -100, '🎉 Good Job!', {
      fontFamily: CONFIG.fontFamily,
      fontSize: '32px',
      color: CONFIG.textColor
    }).setOrigin(0.5);
    
    const levelText = this.scene.add.text(0, -40, `Level ${level} Completed`, {
      fontFamily: CONFIG.fontFamily,
      fontSize: '24px',
      color: CONFIG.textColor
    }).setOrigin(0.5);
  ...
  showGameOver(score) {
    ...
    const titleText = this.scene.add.text(0, -100, 'Game Over', {
      fontFamily: CONFIG.fontFamily,
      fontSize: '32px',
      color: CONFIG.textColor
    }).setOrigin(0.5);
    
    const scoreText = this.scene.add.text(0, -40, `Score: ${score}`, {
      fontFamily: CONFIG.fontFamily,
      fontSize: '24px',
      color: CONFIG.textColor
    }).setOrigin(0.5);
    ...
    // Best score
    const bestScore = localStorage.getItem('bestScore') || 0;
    const bestScoreText = this.scene.add.text(0, 10, `Best: ${bestScore}`, {
      fontFamily: CONFIG.fontFamily,
      fontSize: '24px',
      color: CONFIG.textColor
    }).setOrigin(0.5);
